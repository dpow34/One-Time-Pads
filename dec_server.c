...
int main(int argc, char *argv[]){
  ...
  while(1){
    ...
    char *keyData;
    char *saveptr;
    // splits buffer by the new line so cipherData and keyData contains ciphertext and key data respectively
    cipherData = strtok_r(buffer, "\n", &saveptr);
    keyData = strtok_r(NULL, "\0", &saveptr);
    char *alpha = "ABCDEFGHIJKLMNOPQRSTUVWXYZ ";
    int arr[70000];  // holds integers that correspond to one-time pads math
    int k = 0;  // index for one-time pad math array
    // iterates through each character in cipherData
    while(cipherData[k] != '\0') {
      // loops through all 27 possible characters
      for(int i = 0; i < 28; i++) {
        // character is found in the alpha
        if (alpha[i] == cipherData[k]) {
    ...
    // iterates through each character in keyData
    while(keyData[j] != '\0') {
